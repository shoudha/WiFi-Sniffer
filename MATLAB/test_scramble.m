...
% T_G12 = [   9 0.031 0.000 9 0.033 -0.044 2 -0.002 -0.038 3 -0.081 0.084
...
%             80 -0.109 0.000 2 + ones(1, 9)];
%                 
% T_G12 = T_G12';
% T_G12 = T_G12(:)';
% T_G12(T_G12 > 1) = [];
% sig_re = T_G12(1:2:end);
% sig_im = T_G12(2:2:end);
% sig_tb = sig_re + sqrt(-1)*sig_im;
% sig_tb(end) = [];
...
T_13 = [    9 0 24 0 48 0 72 1 96 0 120 1
            9 0 25 1 49 0 73 0 97 0 121 0
...
T_13 = T_13(:)';
T_13(T_13 > 1) = [];
raw_data_first_144 = T_13;
        



T_14 = [720 0 744 0 768 1 792 1 816 0 840 0
...
T_14 = T_14(:)';
T_14(T_14 > 1) = [];
raw_data_last_144 = T_14;


T_16 = [    9 0  24 1 48 1 72 0 96  0 120 0
            9 1  25 0 49 1 73 1 97  0 121 0
...
T_16 = T_16(:)';
T_16(T_16 > 1) = [];
data_dec_first_144 = T_16;

T_17 = [720 0 744 0 768 1 792 0 816 0 840 0
...
T_17 = T_17(:)';
T_17(T_17 > 1) = [];
data_dec_last_144 = T_17;

T_18 = [9  0 32 1 64 0 96  1 128 1 160 1
        9  0 33 0 65 1 97  0 129 1 161 1
...
T_18(T_18 > 1) = [];
data_dei = T_18;
data_coded_given = data_dei;

T_G21 = [   9  0 32 0 64 0 96  0 128 0 160 0
            9  1 33 1 65 0 97  1 129 0 161 0
...
T_G22 = [   -32 0.000 0.000 -16 -0.949 0.316 9 0.000 0.000 16 -0.316 -0.949
            -31 0.000 0.000 -15 -0.949 -0.949 9 -0.316 0.949 17 -0.949 0.316
            -30 0.000 0.000 -14 -0.949 -0.949 9 0.316 0.949 18 -0.949 -0.949
            ...
            -18 -0.316 -0.949 -9 -0.949 -0.316 14 0.949 -0.316 30 0.000 0.000
            -17 -0.316 0.316 -9 -0.949 0.949 15 0.949 -0.949 31 0.000 0.000];
        
T_G22 = T_G22(:)';
T_G22(abs(T_G22) >2) = [];
data_1_re = [T_G22(1:16), T_G22(33:48), T_G22(65:80), T_G22(97:112)];
data_1_im = [T_G22(17:32), T_G22(49:64), T_G22(81:96), T_G22(113:128)];
data_1 = data_1_re + sqrt(-1)*data_1_im;
        

T_G24 = [   9 0.023 0.023 9 -0.132 0.002 2 -0.013 -0.079 3 0.143 -0.013
            ...
            880 -0.006 0.005 2 + ones(1, 9)];
                

T_G24 = T_G24';
T_G24 = T_G24(:)';
T_G24(T_G24 > 1) = [];
entire_packet_re = T_G24(1:2:end);
entire_packet_im = T_G24(2:2:end);
x = entire_packet_re + sqrt(-1)*entire_packet_im;
...
% plot(x_freq, 'x', 'LineWidth', 3)

% table(real(x1_freq), real(data_1).')

% stem(abs(x1_freq), '.');hold on;plot(abs(data_1), 'o', 'LineWidth', 3)
...
x_q = x_re_q + sqrt(-1).*x_im_q;
x_q = reshape(x_q, 48, 6);

% hold on
% plot(x_q, 'ok', 'LineWidth', 5)
...
% stem(data(1:192), '.');hold on;plot(databit, 'o', 'LineWidth', 3)
% sum(data(1:192) ~= databit.')
...
sum(x_dei(:, 1) ~= data_dei.');


%% Interleave test
coder_input = data_dec_first_144;
expected_coded_output = x_dei(:, 1);

%Rearranging after encoding
trellis = poly2trellis(7, [133 171]);
puncpat = [1,1,1,0,0,1,1,1,1,0,0,1,1,1,1,0,0,1];
[decoder_actual_output_no_punc, final_state] = convenc(coder_input, trellis, puncpat);


decoder_actual_output_no_punc = reshape(decoder_actual_output_no_punc, 12, 16);
actual_coded_output = decoder_actual_output_no_punc;
expected_coded_output = reshape(expected_coded_output, 12, 16);

check = sum(actual_coded_output ~= expected_coded_output);
sum(check);

%% Puncturing and Viterbi Decode

%Insert zeros in groups of symbols and decode entire symbol

all_data_encoded = x_dei; %Not symbol wise
cols = numel(all_data_encoded)/12;
all_data_encoded = reshape(all_data_encoded(:), 12, cols); 

decoder_input = all_data_encoded;
decoder_expected_output_first_144 = data_dec_last_144;
...
decoder_expected_output_first_144 = reshape(decoder_expected_output_first_144, 9, 16);

decoder_actual_output_144 = decoder_actual_output(:, end - 15:end);
% decoder_actual_output_144 = decoder_actual_output(:,1:16);

check = sum(decoder_actual_output_144 ~= decoder_expected_output_first_144);
% sum(check)

%% Descrambler

x_scrambled = decoder_actual_output(:)';
% x_scrambled = reshape(x_scrambled, 144, 6);
scramInit = [1 0 1 1 1 0 1];
    
n = length(x_scrambled);
x_descrambled = zeros(1, n);
for i = 1:n
    save_state_4 = scramInit(4);
    save_state_7 = scramInit(7);
    x_descrambled(i) = xor(x_scrambled(i), xor(scramInit(4), scramInit(7)));
    scramInit = circshift(scramInit',1)';
    scramInit(1)= xor(save_state_4, save_state_7);
end

x_descrambled = reshape(x_descrambled, 144, 6);
x_descrambled = flipud(x_descrambled);


% sum(raw_data_first_144 ~= x_descrambled(:, 1).')
% sum(raw_data_last_144 ~= x_descrambled(:, end).')
...
x_descrambled(1:16) = [];
% x_descrambled(end - 41:end) = [];
% x_descrambled(end - 5:end) = [];
x_descrambled = x_descrambled(1:800);

%Check data bits
% sum(x_descrambled(end - 95:end) ~= raw_data_last_144(1:96))
% sum(x_descrambled(1:128) ~= raw_data_first_144(end - 127:end))

%Convert the databits to hexa and then character
x_raw_data_bin = reshape(x_descrambled, 8, 100).';
x_raw_hex = binaryVectorToHex(x_raw_data_bin);

x_raw_dec = hex2dec(x_raw_hex);
x_raw_char = char(x_raw_dec)'
x_raw_char(25:25+72-1)
